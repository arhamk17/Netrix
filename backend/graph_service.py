...
def write_to_graph(case_id: str, entities: list[dict], relations: list[dict], evidence_id: str):
    with get_neo4j_session() as session:
        ...
        for rel in relations:
            subj_label = _label(rel["subject_label"])
            obj_label = _label(rel["object_label"])
...
def route_get_path(case_id: str = Query(...), from_: str = Query(..., alias="from"),
                    to: str = Query(...), current_user: User = Depends(get_current_user)):
...
def route_get_timeline(case_id: str, current_user: User = Depends(get_current_user)):
